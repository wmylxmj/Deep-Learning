# -*- coding: utf-8 -*-
"""
Created on Tue Aug 28 22:23:33 2018
...
n.Train(n, train_X, train_Y, learning_rate = 0.3, \
...
plot_decision_boundary(lambda x: n.Predict(n, x.T), \
                       train_X, np.squeeze(train_Y))     
n.Query(n, test_X, test_Y)

a = DeepNeuralNetwork
a.__init__(a, [train_X.shape[0], 20, 3, 1])
a.Train_Dropout(a, train_X, train_Y, [0.86, 0.86], \
...
axes.set_xlim([-0.75,0.40])
axes.set_ylim([-0.75,0.65])
plot_decision_boundary(lambda x: a.Predict(a, x.T), \
                       train_X, np.squeeze(train_Y))     
a.Query(a, test_X, test_Y)
