# -*- coding: utf-8 -*-
"""
Created on Sat Sep  8 13:27:25 2018
...
class DeepNeuralNetwork():
    ...
    def Forward_Propagation_TensorFlow_Softmax(self, X, parameters):
        # copy the dataset X (or A0)
        self.dataset = {} 
        self.dataset['X'] = X[:]
        # the number of datasets
    ...
    def Compute_Cost_TensorFlow_Softmax(self, ZL, Y):
        self.dataset['Y'] = Y[:]
    ...
    def Train_TensorFlow_Softmax(self, X_train, Y_train, iterations = 1500, learning_rate = 0.0001, minibatch_size = 32, print_cost = True):
        ...
        parameters = self.Parameters_Init_TensorFlow(self.layer_list)
        ZL = self.Forward_Propagation_TensorFlow_Softmax(X, parameters)
        cost = self.Compute_Cost_TensorFlow_Softmax(ZL, Y)
        ...
        with tf.Session() as sess:
            ...
            for i in range(1, iterations + 1):
                ...
                if print_cost and i % (10*self.iteration_unit) == 0:
                    ...
                    self.Query_Tensorflow_Softmax(X_train, Y_train)
                if i % self.iteration_unit == 0:
    ...
    def Query_Tensorflow_Softmax(self, X, Y):
        correct_prediction = tf.equal(tf.argmax(self.ZL), tf.argmax(Y))
...
parameters = n.tf_parameters
print(parameters)
with tf.Session() as sess:
    X, Y = n.Create_Placeholders_TensorFlow(12288, 6)
    Z3 = n.Forward_Propagation_TensorFlow_Softmax(X, parameters)
    print("Z3 = " + str(Z3))

#n.Train_TensorFlow_Softmax(X_train, Y_train)
