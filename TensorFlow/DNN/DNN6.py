# -*- coding: utf-8 -*-
"""
Created on Tue Sep 11 21:12:29 2018
...
class DeepNeuralNetwork():
    ...
    def __init__(self, identifier, layer_list, name = None, description = None):
        # save the data
        ...
        self.checkpoints_save_dir = "log/" + str(self.identifier) + "/checkpoints/"
        self.tensorboard_save_dir = "log/" + str(self.identifier) + "/tensorboard/"
        # make the path
        ...
        print("The identifier of the deep neural network is '" + str(self.identifier) + "'.")
        print('The layer list is ' + str(self.layer_list) + '.')
        print("The parameters will work under the scope named '" + self.identifier + "'.")
        print("The checkpoints will be saved under 'log/" + str(self.identifier) + "/checkpoints'." )
        print("The tensorboard will be saved under 'log/" + str(self.identifier) + "/tensorboard'." )
        print("\n********************************************")
        pass
    ...
    def Train(self, X_train, Y_train, iterations = 1500, learning_rate = 0.0001, minibatch_size = 32, tensorboard_save_name = "train", ckpt_file_name = "model.ckpt", print_cost = True):
        ...
        'train save name:' + str(tensorboard_save_name) + '\n'
        tf.summary.text('train_details', tf.convert_to_tensor(train_text))
        ...
        with tf.Session() as sess:
            ...
            summary_writer = tf.summary.FileWriter(self.tensorboard_save_dir + tensorboard_save_name + "/", sess.graph)
            print("Now training...")
            for i in range(1, iterations + 1):
                ...
                if i % self.iteration_unit == 0:
                    ...
                    pass
                saver.save(sess, self.checkpoints_save_dir + ckpt_file_name, global_step = i)
                pass
    ...
    def Query(self, X_test, Y_test, checkpoint_file_name = None):
        X = tf.placeholder(tf.float32, shape=[self.n_x, None])
        Y = tf.placeholder(tf.float32, shape=[self.n_y, None])
        ZL = self.Forward_Propagation(X)
        init = tf.global_variables_initializer()
        ...
        with tf.Session() as sess:
            sess.run(init)
            if checkpoint_file_name != None:
                saver.restore(sess, self.checkpoints_save_dir + checkpoint_file_name)
                correct_prediction = tf.equal(tf.argmax(ZL), tf.argmax(Y))
                accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
...
'''
n = DeepNeuralNetwork(identifier = "model_test", layer_list = [12288, 48, 18, 6], name = "DNN1", description = 'test') 
n.iteration_unit = 10
n.Train(X_train, Y_train,  iterations = 900, learning_rate = 0.0005, minibatch_size = 32, tensorboard_save_name = "train_2", ckpt_file_name = "model_2.ckpt", print_cost = True)
'''
n = DeepNeuralNetwork(identifier = "model_test", layer_list = [12288, 48, 18, 6], name = "DNN1", description = 'test') 
n.Query(X_train, Y_train)
n.Query(X_test, Y_test)
n.Query(X_train, Y_train, checkpoint_file_name = 'model_1.ckpt-999')
n.Query(X_test, Y_test, checkpoint_file_name = 'model_1.ckpt-999')
n.Query(X_train, Y_train, checkpoint_file_name = 'model_2.ckpt-897')
n.Query(X_test, Y_test, checkpoint_file_name = 'model_2.ckpt-897')
